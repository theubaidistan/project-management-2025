...
//     // Check if user is alreday a member
...
//     res.json({ member, message: "Member added successully" });
...
export const createProject = async (req, res) => {
  try {
    ...
    // Check workspace and admin role
    const workspace = await prisma.workspace.findUnique({
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "You don't have permission to create projects" });
    }
    ...
    if (team_members?.length > 0) {
      const membersToAdd = workspace.members
        ...
        .map((m) => ({ projectId: project.id, userId: m.user.id }));
...
export const updateProject = async (req, res) => {
  try {
    ...
    // Get new team lead ID if provided
    const teamLeadUser = team_lead
      ? await prisma.user.findUnique({
          where: { email: team_lead },
          select: { id: true },
